    ...
    <div className="space-y-8">
      <div className="space-y-4 animate-fade-in">
        ...
        <p className="text-muted-foreground max-w-3xl">
          Gerencie os diferentes tipos de usuários da plataforma e suas
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {userTypes.map((user, index) => (
          ...
          >
            <CardHeader
              className={`${user.color} flex flex-row items-center justify-between`}
            >
              <CardTitle className={`text-xl ${user.textColor}`}>
                {user.type}
              </CardTitle>
              <div className="h-8 w-8 rounded-full bg-white/20 flex items-center justify-center">
                <span className={`${user.textColor} font-semibold`}>
                  {user.type[0]}
                </span>
              </div>
            </CardHeader>
      ...
      <Card
        className="border border-border mt-10 animate-fade-in"
